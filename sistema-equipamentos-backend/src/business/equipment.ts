...
export async function addEquipment(person_name: string, email: string, type: string, asset_number: string, create_user_id: number) {
    if (!(person_name && email && type && asset_number && create_user_id)) throw new Error("Missing parameters!");

    await equipmentRepository.addEquipment(person_name, email, type, asset_number, Number(create_user_id));
}
...
export async function getEquipmentsById (equipmentId: number|null) {
    if (!equipmentId) throw new Error("Missing equipmentId!");

    const equipmentsInfo = await equipmentRepository.getEquipmentsById(equipmentId);
